...
@pytest.mark.parametrize("entity_type", [EntityType.SUBSCRIPTION, EntityType.PROCESS])
def test_hybrid_carries_full_query_text_embedding_and_window(entity_type: EntityType, monkeypatch) -> None:
    """Hybrid fuzzy-matches on the whole text; its semantic source is the configured window of that entity type."""
...
def test_process_fallback_is_fuzzy_only() -> None:
    """Without an embedding the process retriever has no semantic source and needs no HNSW setting."""
