...
from orchestrator.core.services.parallel import (
...
@pytest.mark.celery
def test_foreach_parallel_worker_dispatches_all_branches(setup_test_celery) -> None:  # noqa: ARG001 — initialises Celery for WORKER-mode dispatch
    ...
    from orchestrator.core.services.parallel import run_worker_branch as _original_run_worker_branch
    ...
    with (
        ...
        patch("orchestrator.core.services.parallel.run_worker_branch", _scoped_run_worker_branch),
        # Stub only the resume dispatch so _join_and_resume's DB logic is tested.
        patch("orchestrator.core.services.processes.get_execution_context", return_value=noop_execution_context),
    ):
...
@pytest.mark.celery
def test_parallel_branch_failure_does_not_retry_with_worker(setup_test_celery) -> None:  # noqa: ARG001 — initialises Celery for WORKER-mode dispatch
    ...
    from test.integration_tests.workflows import store_workflow
...
@pytest.mark.celery
def test_foreach_parallel_single_branch_failure_with_worker(setup_test_celery) -> None:  # noqa: ARG001 — initialises Celery for WORKER-mode dispatch
...
@pytest.mark.celery
def test_worker_resume_after_parallel_executes_remaining_steps(setup_test_celery) -> None:  # noqa: ARG001 — initialises Celery for WORKER-mode dispatch
