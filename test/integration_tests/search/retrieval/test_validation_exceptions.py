...
async def test_invalid_ltree_pattern_raises_error(mock_is_valid, async_session):
    ...
    with pytest.raises(InvalidLtreePatternError):
        await complete_filter_validation(filter_with_invalid_ltree, EntityType.SUBSCRIPTION, async_session)
...
async def test_ltree_filter_takes_special_path(mock_validate_path, mock_lquery_valid, async_session):
    ...
    await complete_filter_validation(filter_with_ltree, EntityType.SUBSCRIPTION, async_session)
