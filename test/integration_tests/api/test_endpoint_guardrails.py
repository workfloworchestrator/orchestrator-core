...
KNOWN_SYNC_ENDPOINTS: frozenset[str] = frozenset(
    {
        ...
        "orchestrator.core.api.api_v1.endpoints.mcp_tools.search_subscriptions_endpoint",
...
def test_no_new_sync_endpoints(fastapi_app):
    sync_endpoints = {
        ...
        if isinstance(route, APIRoute) and not inspect.iscoroutinefunction(inspect.unwrap(route.endpoint))
    }
