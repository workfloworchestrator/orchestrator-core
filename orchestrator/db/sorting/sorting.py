...
def generic_apply_sorting(
    valid_sort_functions_by_column: ValidSortFunctionsByColumnType,
) -> Callable[[QueryType, Iterator[Sort], CallableErrorHander], QueryType]:
    def _apply_sorting(query: QueryType, sort_by: Iterator[Sort], handle_sort_error: CallableErrorHander) -> QueryType:
        for item in sort_by:
            field = item.field
