...
async def export_by_token(token: str) -> dict[str, Any]:
    """Export search results using a token generated by the search agent.

    The token contains entity IDs from a previous search. This endpoint retrieves
    those IDs from Redis, fetches the full entity data from the database, and returns
    flattened records suitable for CSV download.

    Args:
        token: Export token generated by the prepare_export agent tool
    ...
    Raises:
        HTTPException: 404 if token not found or expired, 400 if data is invalid
    """
    async with create_redis_asyncio_client(app_settings.CACHE_URI) as redis_client:

        # Load export data from Redis
        try:
            export_data = await ExportData.from_redis(token, redis_client)
        ...
        try:
            export_records = export_data.fetch_records()
        except ValueError as e:
            raise HTTPException(
