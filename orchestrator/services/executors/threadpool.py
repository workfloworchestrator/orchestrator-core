...
def thread_start_process(
    ...
    # Celery workers use the empty-scope session (no database_scope); close the session
    # after the read so psycopg3's implicit transaction ends. Extract input_state inside
    # the block — the row is detached after close().
    with read_only_transaction(db, logger):
        input_state = retrieve_input_state(pstat.process_id, "initial_state", False).input_state
    pstat.update(state=pstat.state.map(lambda state: StateMerger.merge(state, input_state)))
...
def thread_resume_process(
    ...
    *,
    ...
    # Same idle-in-transaction concern as thread_start_process: close session after reads.
    # Extract input_state inside the block — the row is detached after close().
    with read_only_transaction(db, logger):
        ...
        input_state = retrieve_input_state(process.process_id, retrieve_input_str, False).input_state
