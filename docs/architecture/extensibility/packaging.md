...
For the distribution of the `orchestrator-core`, from version `5.0.0` onward we make use of a namespace package.
...
installed library, without the benefits of having a more comprehensive feature set.
...
For any optional modules that are developed outside of this core library, they can claim any other submodule like
`orchestrator.X.*`.
