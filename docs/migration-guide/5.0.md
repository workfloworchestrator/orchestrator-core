...
Keys that are defined in the `BaseSettings` class will be automatically masked if they contain `"secret"` or
`"password"`.
...
```python
class MySettings(BaseSettings):
    secret_key: str = ""  # Will be masked
    password: str = ""  # Will be masked
    pass: str = ""  # Will NOT be masked
    secure_key: str = ""  # Will NOT be masked
    api_key: SecretStr = ""  # Will be masked
  ```
