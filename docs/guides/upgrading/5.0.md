...
**Be aware of autobegin**: psycopg3 automatically starts a transaction on the first query. If you execute queries outside of an open `database_scope()` (e.g. in custom Celery signal handlers or scheduled tasks), those transactions may not be committed or rolled back, leaving connections in an "idle in transaction" state. Wrap such queries in a fresh scope with an explicit transaction:
...
`database_scope()` creates a fresh SQLAlchemy `Session` bound to a new scope key and cleans it up on exit; `session.begin()` makes the transaction boundary explicit so the connection cannot be left in the "idle in transaction" state. Workflow step code does not need to do this manually — the step framework (`orchestrator.workflow._run_step`) already opens a per-step scope around each step invocation.

> **Removed in this release:** the previous `transactional(db, log)` context manager and the related `disable_commit()` helper have been removed. If you previously imported `from orchestrator.db.database import transactional`, update your code to use `db.database_scope()` + `db.session.begin()` as shown above.
...
For example, if your subclass defines a computed column:
