...
`column_property` attributes from a `SubscriptionTable` subclass onto the base `SubscriptionTable` mapper. This makes
custom computed columns available in queries and GraphQL without modifying the core table definition.

#### What you need to do

If you have a custom `SubscriptionTable` subclass with extra `column_property` definitions, you can now register them
