...
7. Migration from psycopg2 to psycopg3

### 1. GraphQL `scheduledTasks` query no longer returns task created by the decorator

The GraphQL `scheduledTasks` query no longer returns task created by the decorator, but only tasks that are scheduled
via the API.
...
The database driver has been changed from `psycopg2` to `psycopg3` (`psycopg`). This is a significant change that
affects database connection configuration and behavior.

#### What changed

- The dependency `psycopg2-binary` has been replaced by `psycopg[binary]>=3.3.3`
- The SQLAlchemy dialect has changed from `postgresql://` to `postgresql+psycopg://`
- The `psycopg2` import namespace has changed to `psycopg`

**Key behavioral differences between psycopg2 and psycopg3:**

| Aspect                       | psycopg2                 | psycopg3                              |
|------------------------------|--------------------------|---------------------------------------|
| `Session.close()`            | Automatically rolls back | Does **not** rollback                 |
| Parameter binding            | Client-side              | Server-side                           |
| DDL in implicit transactions | Works                    | Requires `AUTOCOMMIT` isolation level |

The most impactful change is that psycopg3 does **not** automatically rollback when a session is closed. This can leave
database connections in an "idle in transaction" state when returned to the connection pool, causing lock contention
between concurrent workers. To mitigate this, orchestrator-core now includes:

- A connection pool `checkin` event listener that detects and forces a rollback on connections returned with active
  transactions
- An explicit rollback in the `database_scope()` cleanup to ensure connections are always returned clean

#### What you need to do

**1. Update your `DATABASE_URI` environment variable**

The connection string must use the `postgresql+psycopg://` dialect:
...
# After
export DATABASE_URI=postgresql+psycopg://nwa:password@localhost:5432/orchestrator-core
```

**2. Update psycopg2 imports**

If you import directly from the database driver anywhere in your codebase, update the import path:
...
**3. Ensure database operations use `database_scope()`**

With psycopg3, any code that accesses the database outside a `database_scope()` context risks leaving connections in "
idle in transaction" state. This is especially important for Celery workers and background tasks. Ensure all database
access is wrapped:
...
**4. DDL operations require AUTOCOMMIT**

If you have test fixtures or scripts that execute DDL statements (`CREATE DATABASE`, `DROP DATABASE`, etc.) using a bare
SQLAlchemy engine, you need to set `isolation_level="AUTOCOMMIT"`:

```python
# Before (worked with psycopg2)
engine = create_engine(url)

# After (required for psycopg3)
engine = create_engine(url, isolation_level="AUTOCOMMIT")
```

This is only needed for engines that execute DDL. The application engine used by orchestrator-core uses normal
